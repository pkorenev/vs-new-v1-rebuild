...
class ApplicationController < ActionController::Base
  ...
  def get_menu
  	...
  	arr[1] = {:data_url => service_list_path, :id => 'li-service', :title => 'ЩО - Наші послуги', :is_active => request[:controller] == 'page' && request[:action] == 'service', :header => d('main_menu.articles.header'), :description => d('main_menu.services.description')}
